...
appointmentRouter.get("/", async (req, res, next) => {
  ...
  try {
    ...
    const foundAppointments = await Appointment.find(query)
      .populate("doctorId")
      .populate({
        path: "doctorId",
        populate: [
...
appointmentRouter.post("/", async (req, res, next) => {
  ...
  try {
    ...
    if (!availableAppointment) {
      return res
        .status(404)
        .json({ message: "No hay cita disponible para actualizar" });
    }
...
appointmentRouter.delete("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      message: `Se eliminaron ${deleteResult.deletedCount} registros correctamente`,
    });
  } catch (error) {
    next(error);
  }
